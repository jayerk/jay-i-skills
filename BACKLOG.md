...
| # | Status | Title | Description |
|---|--------|-------|-------------|
| 63 | `done` | Dependencies map | Created `frameworks/dependency-mapping/FRAMEWORK.md` (reusable framework with 4+1 dependency type taxonomy, 3-level strength scale, dependency debt concept). Applied to product standards: 23 sub-practice relationships (adoption view with 4-wave sequence) and 38 standard-level cross-references (reference view with coaching guide). Two Mermaid diagrams in `knowledge/product-standards-dependencies-adoption.md` and `knowledge/product-standards-dependencies-reference.md`. Vocabulary updated with 5 new terms. |
