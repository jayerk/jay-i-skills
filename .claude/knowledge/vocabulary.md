...
| Term | Definition |
|------|-----------|
| **Balanced Breakthrough** | A two-part investment framework. **Prioritization** uses Desirability (do users/the business want this?), Feasibility (can we build it?), and Viability (is it sustainable?) to evaluate opportunities. **Capitalization** classifies investments as Run, Improve, or Transform to describe the investment type. D/F/V drives what to do; R/I/T describes how to categorize it. Allocation ratios shift by lifecycle stage — a Growth product invests differently than a Maturity product. |
| **Desirability / Feasibility / Viability** | The three prioritization lenses within Balanced Breakthrough. Desirability = do users and the business want this? Feasibility = can we build it given constraints? Viability = is it sustainable and does it make economic sense? Every opportunity should be evaluated across all three. |
| **Run / Improve / Transform** | The three capitalization categories within Balanced Breakthrough. Run = operational maintenance. Improve = incremental enhancements. Transform = breakthrough bets that change the trajectory. Describes the investment type, not the prioritization. Every initiative should be labeled, and totals should match the intended allocation. |
