...
| 64 | `done` | Applicability master matrices | Three files: main applicability matrix (`product-standards-applicability.md`) with "find your row" lookup, sub-practice and standard-level matrices (39 standards × 5 stages × 3 product types), inline heatmap Mermaid; lifecycle view (`product-standards-applicability-lifecycle.md`) with 5 expected/stretch diagrams; pillar view (`product-standards-applicability-pillar.md`) with 3 progression diagrams. Cross-referenced from `product-standards.md` Dependencies section. |
| 65 | `done` | Scorecard template | `templates/SCORECARD-TEMPLATE.md`: Two-level assessment template (10-row sub-practice summary + 39-row standard-level detail). Context fields for lifecycle stage and product type drive target lookup. Embedded target reference table (10×5), scoring guide with gap interpretation, adoption wave prioritization logic, and cross-references to all product standards artifacts. |
| 66 | `done` | Universal adoption path | `knowledge/product-standards-adoption.md`: Team-type-aware, assessment-driven adoption guide. Team Type × Domain matrix (4 TT types × 3 domains) shapes emphasis per sub-practice. Capacity sizing (constrained/moderate/full). 4-wave onboarding with starting standards first, expansion standards after Level 2+. Per-team-type entry points within each wave. Three horizons (Month 1-2, 6 months, embedded) with specific milestones. Stall signals at each horizon with root cause diagnosis. System-level embedded state description. 10-row quick reference table. |
| 67 | `done` | Sub-practice–to–repo map | Created `knowledge/product-standards-repo-map.md`: 10-row routing table mapping each sub-practice to primary framework, primary skill, key knowledge files, and per-level coaching guidance. Secondary skill cross-reference table for work-mode handoffs. Cross-referenced from `product-standards.md` Dependencies section and `SCORECARD-TEMPLATE.md` Cross-References. |
...
| 38 | P2 | `done` | Template branding and consistency pass | Normalized all 5 base templates: unified header block (`jay-i-skills template · Type`), ISO date fields, consistent Author naming, added missing Status to Context template, fixed Summary's stale ROADMAP.md → BACKLOG.md reference. Created `_branding.md` for work and PMI Buffalo contexts — visual standards, table rules, section naming conventions, formatting conventions, and tone guidance specific to each persona. |
| 111 | P2 | `todo` | Personal context branding guide | Create `.claude/skills/personal/_branding.md` — visual and formatting standards for personal context deliverables (soup-feast, personal productivity, etc.). Casual tone, minimal structure, low-friction formatting. |
